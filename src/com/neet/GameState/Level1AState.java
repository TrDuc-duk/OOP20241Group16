...
import com.neet.Entity.Enemies.GreyGem;
import com.neet.Entity.Enemies.BlueGem;
import com.neet.Entity.Enemies.Captain;
...
public class Level1AState extends GameState {
	...
	private static final long NOTIFICATION_DURATION = 2000; // 1 giây
	...
	private void populateEnemies() {
		...
		gg = new GreyGem(tileMap, player);
		gg.setPosition(2940, 100);
		enemies.add(gg);
		...
		cap = new Captain(tileMap, player);
		cap.setPosition(2177, 95);
		enemies.add(cap);
		...
		blg = new BlueGem(tileMap, player);
		blg.setPosition(3640, 100);
		enemies.add(blg);
	}
	...
	public void update() {
		...
		for (int i = 0; i < enemies.size(); i++) {
		    ...
		    e.update();
		    if (e.isDead()) {
		        if (e instanceof GreyGem) {
		            // Thêm thông báo khi giết GreyGem
		            notificationText = "Mở khóa kỹ năng: Lướt tấn công!";
		            notificationStartTime = System.nanoTime();
		        }
		        if (!(e instanceof GreyGem || e instanceof BlueGem)) {
	...
	public void draw(Graphics2D g) {
		...
		if (notificationText != null) {
		    long elapsed = (System.nanoTime() - notificationStartTime) / 1_000_000; // Đổi sang ms
		    if (elapsed < NOTIFICATION_DURATION) {
		        ...
		        g.drawString(notificationText, x, 60); // Cập nhật x
		    } else {
