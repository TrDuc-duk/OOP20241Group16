...
import com.neet.Entity.Enemies.RedGem;
...
public class Level1CState extends GameState {
	...
	// Biến để hiển thị thông báo khi RedGem bị hạ
	private String notificationText = null; // Nội dung thông báo
	private long notificationStartTime;     // Thời gian bắt đầu hiển thị
	private static final int NOTIFICATION_DURATION = 3000; // Thời gian hiển thị (ms)
	...
	private void populateEnemies() {
		...
		// Thêm redgem
		rg = new RedGem(tileMap, player);
		rg.setPosition(160, 160);
		enemies.add(rg);
	}
	...
	public void update() {
		...
		for (int i = 0; i < enemies.size(); i++) {
		    ...
		    if (e.isDead() || e.shouldRemove()) {
		        // Kiểm tra nếu là RedGem
		        if (e instanceof RedGem) {
		            notificationText = "Đừng hòng lấy viên ngọc!";
		            notificationStartTime = System.currentTimeMillis(); // Lưu thời gian bắt đầu hiển thị
		        }
	...
	public void draw(Graphics2D g) {
		...
		if (notificationText != null) {
		    long elapsed = System.currentTimeMillis() - notificationStartTime; // Tính thời gian đã trôi qua
		    if (elapsed < NOTIFICATION_DURATION) {
		        g.setFont(new java.awt.Font("Arial", java.awt.Font.BOLD, 14)); // Font chữ
		        g.setColor(java.awt.Color.RED); // Màu chữ
		        
		        // Tính toán vị trí X để căn giữa
		        java.awt.FontMetrics fm = g.getFontMetrics();
		        int textWidth = fm.stringWidth(notificationText);
		        int x = (GamePanel.WIDTH - textWidth) / 2; // Căn giữa theo chiều ngang
		        
		        g.drawString(notificationText, x, 60); // Vẽ dòng chữ
		    } else {
		        notificationText = null; // Ẩn thông báo sau thời gian quy định
		    }
		}
